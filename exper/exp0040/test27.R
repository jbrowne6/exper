...
for(exp in possExp){
    ...
    for(protType in 1:2){
        ...
        if(Experiment == "RF" & NoProt){
            re1000p <- data.frame(type="1000p", specificity=specificity2[ordSpec], sensitivity=sensitivity2[ordSpec], cutval=PossibleCutOffs)
            save(re1000p, file="1000p.Rdata")

            lab <- labs(title="Random Forest 1000 Trees, without Midkine and Prolactin")
            png(file="1000p.png", width=pWidth, height=pHeight)
            ...
            png(file="1000p.a.png", width=pWidth, height=pHeight)
            print(ggplot(ress4, aes(x=cutval, y=val, group=type, colour=type)) + geom_line() + leg + lab)
            dev.off()
            ##############
        }else if(Experiment == "RF" & !NoProt){
            re1000 <- data.frame(type="1000", specificity=specificity2[ordSpec], sensitivity=sensitivity2[ordSpec], cutval=PossibleCutOffs)
            save(re1000, file="1000.Rdata")
            ...
            lab <- labs(title="Random Forest 1000 Trees", subtitle=paste("Sensitivity and Specificity as a function of Cut Val\ncut val is proportion of trees that vote cancer"))
            png(file="1000.a.png", width=pWidth, height=pHeight)
            print(ggplot(ress4, aes(x=cutval, y=val, group=type, colour=type)) + geom_line() + leg + lab)
            dev.off()
            ##############
        }else if(Experiment == "R/L" & NoProt){
            re1000RLp <- data.frame(type="1000RLp", specificity=specificity2[ordSpec], sensitivity=sensitivity2[ordSpec], cutval=PossibleCutOffs)
            save(re1000RLp, file="1000RLp.Rdata")

            lab <- labs(title="Random Forest 1000 Trees", subtitle= "No Midkine or Prolactin, R/L Constraint")
            png(file="1000RLp.png", width=pWidth, height=pHeight)
            print(ggplot(ress1, aes(x=specificity, y = sensitivity)) + geom_line() + leg + lab)
            dev.off()

            lab <- labs(title="Random Forest 1000 Trees", subtitle=paste("Sensitivity and Specificity as a function of Cut Val\ncut val is proportion of trees that vote cancer\n No Midkine or Prolactin, R/L Constraint"))
            png(file="1000RLp.a.png", width=pWidth, height=pHeight)
            print(ggplot(ress4, aes(x=cutval, y=val, group=type, colour=type)) + geom_line() + leg + lab)
            dev.off()

            ##############
        }else if(Experiment == "R/L" & !NoProt){
            re1000RL <- data.frame(type="1000RL", specificity=specificity2[ordSpec], sensitivity=sensitivity2[ordSpec], cutval=PossibleCutOffs)
            save(re1000RL, file="1000RL.Rdata")

            lab <- labs(title="Random Forest 1000 Trees", subtitle= "R/L Constraint")
            png(file="1000RL.png", width=pWidth, height=pHeight)
            print(ggplot(ress1, aes(x=specificity, y = sensitivity)) + geom_line() + leg + lab)
            dev.off()

            lab <- labs(title="Random Forest 1000 Trees", subtitle=paste("Sensitivity and Specificity as a function of Cut Val\ncut val is proportion of trees that vote cancer\nR/L Constraint"))
            png(file="1000RL.a.png", width=pWidth, height=pHeight)
            print(ggplot(ress4, aes(x=cutval, y=val, group=type, colour=type)) + geom_line() + leg + lab)
            dev.off()
            ##############
            ...
            lab <- labs(title="Random Forest 1000 Trees", subtitle= "No Midkine or Prolactin, R/S Constraint")
            png(file="1000RSp.png", width=pWidth, height=pHeight)
            print(ggplot(ress1, aes(x=specificity, y = sensitivity)) + geom_line() + leg + lab)
            dev.off()

            lab <- labs(title="Random Forest 1000 Trees", subtitle=paste("Sensitivity and Specificity as a function of Cut Val\ncut val is proportion of trees that vote cancer\n No Midkine or Prolactin, R/S Constraint"))
            png(file="1000RSp.a.png", width=pWidth, height=pHeight)
            print(ggplot(ress4, aes(x=cutval, y=val, group=type, colour=type)) + geom_line() + leg + lab)
            dev.off()

            ##############
        }else if(Experiment == "R/S" & !NoProt){
            re1000RS <- data.frame(type="1000RS", specificity=specificity2[ordSpec], sensitivity=sensitivity2[ordSpec], cutval=PossibleCutOffs)
            save(re1000RS, file="1000RS.Rdata")

            lab <- labs(title="Random Forest 1000 Trees", subtitle= "R/S Constraint")
            png(file="1000RS.png", width=pWidth, height=pHeight)
            print(ggplot(ress1, aes(x=specificity, y = sensitivity)) + geom_line() + leg + lab)
            dev.off()

            lab <- labs(title="Random Forest 1000 Trees", subtitle=paste("Sensitivity and Specificity as a function of Cut Val\ncut val is proportion of trees that vote cancer\nR/S Constraint"))
            png(file="1000RS.a.png", width=pWidth, height=pHeight)
            print(ggplot(ress4, aes(x=cutval, y=val, group=type, colour=type)) + geom_line() + leg + lab)
            dev.off()
