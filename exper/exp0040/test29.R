    ...
    for(protType in 1:2){
        ...
        for(ps in Depths){
            ...
            for(i in 1:10){
                ...
                # Grow forest       
                forest <- RerF(Xtr,Ytr,min.parent =3, max.depth=ps, trees=trees, seed=sample(1:10000,1),mat.options = list(p = ncol(X), d =ceiling(sqrt(ncol(X))), random.matrix = "rf", rho = 1/ncol(X)))
                predictionTotals <- Predict(Xte,forest, aggregate.output = FALSE)
        ...
        ress1 <- data.frame(specificity=specificity2[ordSpec], sensitivity=sensitivity2[ordSpec], cutval=PossibleCutOffs, TD=treeDepth[ordSpec])
...
#        }
        ...
        if(Experiment == "RF" & NoProt){
            lab <- labs(title="Random Forest 1000 Trees without Midkine and Prolactin", subtitle="variable size Depth")
            png(file="1000D_p.png", width=pWidth, height=pHeight)
            ##############
        }else if(Experiment == "RF" & !NoProt){
            lab <- labs(title="Random Forest 1000 Trees", subtitle="variable size Depth")
            png(file="1000D.png", width=pWidth, height=pHeight)
            ##############
        }else if(Experiment == "R/L" & NoProt){
            lab <- labs(title="Random Forest 1000 Trees", subtitle="R/L, omit, variable size Depth")
            png(file="1000D_RLp.png", width=pWidth, height=pHeight)
            ##############
        }else if(Experiment == "R/L" & !NoProt){
            lab <- labs(title="Random Forest 1000 Trees", subtitle="R/L, variable size Depth")
            png(file="1000D_RL.png", width=pWidth, height=pHeight)
            ##############
        }else if(Experiment == "R/S" & NoProt){
            lab <- labs(title="Random Forest 1000 Trees", subtitle="R/S, omit, variable size Depth")
            png(file="1000D_RSp.png", width=pWidth, height=pHeight)
            ##############
        }else if(Experiment == "R/S" & !NoProt){
            lab <- labs(title="Random Forest 1000 Trees", subtitle="R/S, variable size Depth")
            png(file="1000D_RS.png", width=pWidth, height=pHeight)
        }
        ##############
        print(ggplot(ress1, aes(x=specificity, y = sensitivity, colour=TD)) + geom_line(aes(linetype=TD)) + leg + lab)
        dev.off()
