...
possExp <- c( "R/L", "R/S","RF")
    ...
    for(protType in 1:2){
        ...
        treeDepth <- NA
        tVal <- 1/ncol(X)/2 
        Depths <- c(tVal,tVal*2,tVal*3,tVal*4,tVal*5,tVal*6,tVal*7,tVal*8)
        SpecMatrix <- matrix(10*numCuts*length(Depths), nrow=numCuts*length(Depths), ncol=10)
        SensMatrix <- matrix(10*numCuts*length(Depths), nrow=numCuts*length(Depths), ncol=10)
        PredMatrix <- matrix(10*numCuts*length(Depths), nrow=numCuts*length(Depths), ncol=10)
        ...
        for(ps in Depths){
            ...
            for(i in 1:10){
                ...
                # Grow forest       
                forest <- RerF(Xtr,Ytr,min.parent =3, max.depth=8, trees=trees, seed=sample(1:10000,1),mat.options = list(p = ncol(X), d =ceiling(sqrt(ncol(X))/2*3), random.matrix = "rf", rho =ps ))
                predictionTotals <- Predict(Xte,forest, aggregate.output = FALSE)
        ...
        sensitivity2 <- NA
        pred2 <- NA
        for(z in 1:(numCuts*length(Depths))){
        ...
        ress1 <- data.frame(specificity=specificity2[ordSpec], sensitivity=sensitivity2[ordSpec], cutval=PossibleCutOffs, rho=treeDepth[ordSpec])
...
#        }
        ...
        if(Experiment == "RF" & NoProt){
            lab <- labs(title="Random Forest 1000 Trees without Midkine and Prolactin", subtitle="variable size rho")
            png(file="1000rh_p.png", width=pWidth, height=pHeight)
            ##############
        }else if(Experiment == "RF" & !NoProt){
            lab <- labs(title="Random Forest 1000 Trees", subtitle="variable size rho")
            png(file="1000rh.png", width=pWidth, height=pHeight)
            ##############
        }else if(Experiment == "R/L" & NoProt){
            lab <- labs(title="Random Forest 1000 Trees", subtitle="R/L, omit, variable size rho")
            png(file="1000rh_RLp.png", width=pWidth, height=pHeight)
            ##############
        }else if(Experiment == "R/L" & !NoProt){
            lab <- labs(title="Random Forest 1000 Trees", subtitle="R/L, variable size rho")
            png(file="1000rh_RL.png", width=pWidth, height=pHeight)
            ##############
        }else if(Experiment == "R/S" & NoProt){
            lab <- labs(title="Random Forest 1000 Trees", subtitle="R/S, omit, variable size rho")
            png(file="1000rh_RSp.png", width=pWidth, height=pHeight)
            ##############
        }else if(Experiment == "R/S" & !NoProt){
            lab <- labs(title="Random Forest 1000 Trees", subtitle="R/S, variable size rho")
            png(file="1000rh_RS.png", width=pWidth, height=pHeight)
        }
        ##############
        print(ggplot(ress1, aes(x=specificity, y = sensitivity, colour=rho)) + geom_line(aes(linetype=rho)) + leg + lab)
        dev.off()
