...
for(i in 1:10){
    ...
    if(length(Xtr[,1])+length(Xte[,1]) != length(X[,1])){
        print(paste("training#= ", length(Xtr[,1]), ", test#= ", length(Xte[,1]), "total#= ", length(X[,1])))
...
tVal <- 1/ncol(X)/6 
#rhV <- c(tVal,tVal*2,tVal*3,tVal*4,tVal*5,tVal*6,tVal*7,tVal*8)
#rhV <- 1:12 * tVal
rhV <- 10000
...
for(mp in mpV){
    ...
    #        for(rh in rhV){
                    ...
                    for(i in 1:10){
                        ...
                        forest <- RerF(Xtr,Ytr,min.parent =mp, max.depth=md, trees=treeN, seed=sample(1:10000,1),mat.options = list(p = ncol(X), d =mtry, random.matrix = "rf", rho =5 ))
                        predictionTotals <- Predict(Xte,forest, aggregate.output = FALSE)
                        ...
                        if(specificity2 > .99){
                            if(sensitivity2 > maxSens){
                               ...
                               # maxrh <- rh
                                ...
                                maxtree <- treeN
                                print(paste("max sens= ", maxSens, " maxParent= ", mp, "maxDepth= ", md, "mtry= ", mtry, "cutOff= ", maxCV, " trees= ", treeN))
