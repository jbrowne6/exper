    ...
    function(X, Y, min.parent, max.depth, bagging, replacement, stratify, class.ind, class.ct, fun, mat.options, store.oob, store.ns, progress, rotate){
        ...
        while (CurrentNode < NextUnusedNode) {
            ...
            while (nz.idx <= nnz) {
                ...
                #numCancer <- sum(y[1L:NdSize[CurrentNode]]==2)
                #numNon <- length(y[1L:NdSize[CurrentNode]]) - numCancer

                RVines <- FALSE
                if (RVines == TRUE){
                ##################################################################
                #                    Find Best Split RVines
                ##################################################################
...
if(xInd >= 1){
                  ...
                  ########################################################
                  ########### Modified RF ##################################
                  # Next six lines are used for exper 22.
                  # bigX <- which(x[1L:NdSize[CurrentNode]] > min(X[,sparseM[lrows,1L]]))
                  # x <- x[1L:NdSize[CurrentNode]][bigX] 
                  # y <- y[1L:NdSize[CurrentNode]][bigX] 
                  # ret[] <- findSplit(x = x,
                  #                    y = y,
                  #                    ndSize = length(y),
...
#R/L Test
            # if(sum(Y[Assigned2Node[[NextUnusedNode+1]]]==2)/sum(Y[Assigned2Node[[NextUnusedNode]]]==2) < sum(Y[Assigned2Node[[NextUnusedNode+1]]]==1)/sum(Y[Assigned2Node[[NextUnusedNode]]]==1)){
#R/S Test
   #          if(sum(Y[Assigned2Node[[NextUnusedNode+1L]]]==2)/sum(Y[Assigned2Node[[CurrentNode]]]==2) < sum(Y[Assigned2Node[[NextUnusedNode+1]]]==1)/sum(Y[Assigned2Node[[CurrentNode]]]==1)){
            #   stop(paste("condition error: numC= ", sum(Y[Assigned2Node[[NextUnusedNode+1]]]==2), ", numCanTot= ",sum(Y[Assigned2Node[[CurrentNode]]]==2),", numNonCan= ", sum(Y[Assigned2Node[[NextUnusedNode+1]]]==1), ", numNonTot= ", sum(Y[Assigned2Node[[CurrentNode]]]==1), ", ndsize=", NdSize[CurrentNode]))
            # }
