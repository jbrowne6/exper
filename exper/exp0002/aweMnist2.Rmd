...
for (p in 2:ML){
		...
		for (i in 1:nTimes){
			gc()
			ptm <- proc.time()
			forest <- rfr(Xr[1:numS,],Yr[1:numS], trees=num_trees, bagging=.3, MinParent=1, MaxDepth="inf",COOB=TRUE, stratify=TRUE, NumCores=q)
			ptm_hold[i] <- (proc.time() - ptm)[3]
			forest[i] <- object.size(forest)
		}
		line_type[entry]<-"Random (20000x784)"
		cores_used[entry] <- q
		median_time[entry]<- median(ptm_hold)
		speed_up[entry]<-2*median_time[initial_run_entry]/median_time[entry]
        entry<-entry+1
...
if(run_su){
save(ress1, file="aweMnist2_speed.Rdata")
...
numS <- floor(facm)
		...
		for (i in 1:nTimes){
			ptm <- proc.time()
			forest <- rfr(Xr[1:numS,],Yr[1:numS], trees=num_trees, bagging=0, MinParent=1, MaxDepth="inf",COOB=TRUE, stratify=TRUE, NumCores=q)
			ptm_hold[i] <- (proc.time() - ptm)[3]
			forest[i] <- object.size(forest)
		}
line_type[entry] <- "Random"
		cores_used[entry] <- q
		median_time[entry]<- median(ptm_hold)
		scale_up[entry]<-median_time[initial_run_entry]/median_time[entry] 
		entry = entry+1
...
save(ress2, file="aweMnist2_scale.Rdata")
}
```
