...
for (i in 1:nTimes){
    ...
    forest <- RerF(X,Y, trees=num_trees, bagging=.3, min.parent=1, max.depth=6, store.oob=TRUE, stratify=TRUE, num.cores=num.threads, seed=sample(1:100000,1) )
			ptm <- proc.time()
...
print(g <- ggplot(ress1, aes(Dataset, value,color = variable)) + geom_bar(stat="identity")+ leg + labs(title="Test Times Single Core", x="Dataset", y="Test Time (s) ", subtitle=paste("")))
